...
| opmaak-verzoek (markeren, vet, cursief, kleur) | kán sinds v14 via een format-suggestie (`action: "format"`); markeren valt buiten Words revisies → Afwijzen draait hem expliciet terug |
| structuurverzoek (kolommen, afbeeldingen, lettergrootte, stijlen) | kán niet — "doen alsof" is het risico (no-op-vangnet + eerlijke uitleg) |
| artikel hernoemen + verwijzingen ("artikel 5", "art. 5", "Art. 5 lid 2", tabel, bijlage) | cascade: gemiste of dubbele verwijzingen; volledige recall vereist |
...
| Bestand | Inhoud |
|---|---|
| `corpus.mjs` | 14 realistische NL-documenten: arbeids-, huur-, NDA-, AV-, VSO-, opdracht-, aandeelhouders-, verwerkers-, SaaS/licentie-, franchiseovereenkomst (Bijlage 1 + typografie-traps), gegenereerde raamovereenkomst (28k), dienstverleningsovereenkomst met **tab-tabel**, leveringsvoorwaarden met **speciale tekens** (§ — … ½ ’ diacritiek), samenwerkingsovereenkomst met **kruisverwijzingen in vier schrijfwijzen** (hernummering-cascade) |
| `cases.mjs` | 93 instructies + verwachtingen, elk gelabeld met `failureMode` |
| `lib/score.mjs` | scoring: find-validatie (incl. tab-celgrens), scope-precisie/recall (bijlage-bewust: `'B1.2'`), waarde, `format`-verwachting (opmaak-suggesties), `findMust`/`findMustNot`, reply |
